...
contains
        subroutine load_ggml(filename, w, c, vocab, token_lengths, v)
        ...
        open(UNIT=5, FILE=filename, FORM="UNFORMATTED",&
                ...
                do i = 1,kv_pairs
                        ...
                        else if (keys(i) .eq. "distilbert.block_count") then
                                num_layers = values(i)%i32 !assume it's int(4)
                        else if (keys(i) .eq. "distilbert.embedding_length") then
                                emb_length = values(i)%i32
                        else if (keys(i) .eq. "distilbert.attention.head_count") then
                                head_count = values(i)%i32
                        else if (keys(i) .eq. "distilbert.context_length") then
                                context_length = values(i)%i32
                        else if (keys(i) .eq. "tokenizer.ggml.tokens") then
                                vocab_size = (size(values(i)%a))
                        else if (keys(i) .eq. "distilbert.attention.head_count_kv") then
                                kv_heads = values(i)%i32
                        else if (keys(i) .eq. "distilbert.feed_forward_length") then
                ...
                w%word_embeddings = temp_gt%f322d

                if (verbose) then
                        print *, "loaded word embedding weights:", size(w%word_embeddings)
                end if
       
                t0 = tensor_by_name("position_embd.weight")
                temp_gt = read_layer(5,t0,file_pos)

                !call write_tensor(8,temp_gt)
                w%position_embeddings = temp_gt%f322d

                if (verbose) then
                        print *, "loaded position embedding weights:", size(w%position_embeddings)
                end if
                ...
                t0 = tensor_by_name("token_embd_norm.weight")
                temp_gt = read_layer(5,t0,file_pos)

                !call write_tensor(8,temp_gt)
                w%emb_layer_norm_w = temp_gt%f321d

                if (verbose) then
                        print *, "loaded embedding layernorm weights:", size(w%emb_layer_norm_w)
                end if

                t0 = tensor_by_name("token_embd_norm.bias")
                temp_gt = read_layer(5,t0,file_pos)

                !call write_tensor(8,temp_gt)
                w%emb_layer_norm_b = temp_gt%f321d

                if (verbose) then
                        print *, "loaded embedding layernorm bias:", size(w%emb_layer_norm_b)
                end if


                allocate(w%wq(emb_length,emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%wq(:,:,i) = temp_gt%f322d
                end do 

                if (verbose) then
                        print *, "loaded wq weights:", size(w%wq)
                end if


                allocate(w%bq(emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bq(:,i) = temp_gt%f321d
                end do


                if (verbose) then
                        print *, "loaded wq bias:", size(w%bq)
                end if

                allocate(w%wk(emb_length,emb_length,num_layers)) 
                do i = 1,num_layers
                        ...
                        ! f16 
                        !call write_tensor(8,temp_gt)
                        w%wk(:,:,i) = temp_gt%f322d
                end do

                if (verbose) then
                        print *, "loaded wk weights:", size(w%wk)
                end if

                allocate(w%bk(emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bk(:,i) = temp_gt%f321d
                end do


                if (verbose) then
                        print *, "loaded wk bias:", size(w%bk)
                end if


                allocate(w%wv(emb_length,emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%wv(:,:,i) = temp_gt%f322d
                end do

                !print *, "qkv sum: ", sum(w%wqkv)
                if (verbose) then
                        print *, "loaded wv weights:", size(w%wv)
                end if

                allocate(w%bv(emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bv(:,i) = temp_gt%f321d
                end do


                if (verbose) then
                        print *, "loaded wv bias:", size(w%bv)
                end if
                ...
                allocate(w%bo(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".attn_output.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%bo(:,i) = temp_gt%f321d
                end do


                if (verbose) then
                        print *, "loaded wo bias:", size(w%bo)
                end if



                allocate(w%sa_layer_norm_w(emb_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! should be f32
                        !call write_tensor(8,temp_gt)
                        w%sa_layer_norm_w(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded sa layernorm weights:", size(w%sa_layer_norm_w)
                end if

                allocate(w%sa_layer_norm_b(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".ffn_norm.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! should be f32
                        !call write_tensor(8,temp_gt)
                        w%sa_layer_norm_b(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded sa layernorm bias:", size(w%sa_layer_norm_w)
                end if



                allocate(w%w1(emb_length,ffn_length,num_layers))
                do i = 1,num_layers
                        ...
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%w1(:,:,i) = temp_gt%f322d
                end do

                if (verbose) then
                        print *, "loaded w1 weights:", size(w%w1)
                end if

                allocate(w%b1(ffn_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".ffn_up.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%b1(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded w1 bias:", size(w%b1)
                end if
                ...
                allocate(w%b2(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".ffn_down.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! f16
                        !call write_tensor(8,temp_gt)
                        w%b2(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded w2 (down) bias:", size(w%b2)
                end if

                allocate(w%out_layer_norm_w(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".output_norm.weight"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! should be f32
                        !call write_tensor(8,temp_gt)
                        w%out_layer_norm_w(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded output norm weights:", size(w%out_layer_norm_w)
                end if

                allocate(w%out_layer_norm_b(emb_length,num_layers))
                do i = 1,num_layers
                        write(tempstr,"(A,I0,A)") "blk.", i-1, ".output_norm.bias"
                        t0 = tensor_by_name(tempstr)
                        temp_gt = read_layer(5,t0,file_pos)
                        ! should be f32
                        !call write_tensor(8,temp_gt)
                        w%out_layer_norm_b(:,i) = temp_gt%f321d
                end do

                if (verbose) then
                        print *, "loaded output norm bias:", size(w%out_layer_norm_w)
                end if
                ...
                t0 = tensor_by_name("output")
                temp_gt = read_layer(5,t0,file_pos)
                ! f16
                !call write_tensor(8,temp_gt)
                w%linear = temp_gt%f322d

                if (verbose) then
                        print *, "loaded classifier weights:", size(w%linear)
                end if
